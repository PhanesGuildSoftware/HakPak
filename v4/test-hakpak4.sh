...
echo -e "${CYAN}HakPak4 Test Suite${NC}\n"

PASSED=0
FAILED=0
APP_VERSION="$(tr -d '[:space:]' < "$SCRIPT_DIR/VERSION" 2>/dev/null || echo "")"
...
if [[ -f "$SCRIPT_DIR/hakpak4.py" ]] && \
   [[ -f "$SCRIPT_DIR/hakpak4_core.py" ]] && \
...
if [[ -x "$SCRIPT_DIR/hakpak4.sh" ]] && \
   [[ -x "$SCRIPT_DIR/install-hakpak4.sh" ]]; then
...
if [[ -n "$APP_VERSION" ]] && python3 "$SCRIPT_DIR/hakpak4.py" --version 2>&1 | grep -q "$APP_VERSION"; then
...
if python3 -m py_compile "$SCRIPT_DIR/hakpak4.py" 2>/dev/null && \
   python3 -m py_compile "$SCRIPT_DIR/hakpak4_core.py" 2>/dev/null; then
...
if python3 -c "import sys; sys.path.insert(0, '$SCRIPT_DIR'); from hakpak4 import Shell, SystemInfo, Tool, ToolCategory" 2>/dev/null; then
...
if [[ $FAILED -eq 0 ]]; then
    ...
    echo -e "${CYAN}HakPak4 is ready to use.${NC}"
    echo ""
    echo "Next steps:"
    echo "  1. Install PyYAML: pip3 install pyyaml"
    echo "  2. Run installer: sudo bash install-hakpak4.sh"
    echo "  3. Launch HakPak4: sudo hakpak4"
    exit 0
else
